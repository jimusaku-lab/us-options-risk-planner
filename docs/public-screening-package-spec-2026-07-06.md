  ...
  "candidates": [
    {
      ...
      "market": "US",
      "underlyingPrice": 100,
      "priceAsOf": "2026-07-06T09:00:00+09:00"
    }
  ]
}
```
...
| `capital` | `availableCashUSD`、`buyingPowerUSD`、`maxLossToleranceUSD`、`assignmentCapitalAvailableUSD`、`stockShares`、`allowAssignment`、`allowStockCalledAway`、`exitRuleConfirmed` など。 |
...
`positionDrafts[].status` は次の3値を使います。

| status | 意味 |
| --- | --- |
| `not_ready` | Bid/Ask、資金、100株、許容損失、保守価格などが不足し、入力候補として確認できない。 |
| `manual_review_required` | データは一部揃っているが、割当、出口ルール、既存株、上級戦略などの手動確認が必要。 |
| `draft_ready` | レビュー前チェックを始められる。注文可能という意味ではない。 |

共通ロジックで生成した建玉案には、`capital`、`exitPlan`、`reviewState` が付きます。`reviewState.checklist` の必須項目が未確認の間は、建玉入力へ進める前に画面上で止めます。
...
このサンプルは、Level 1からLevel 4、Lastのみ、Bid/Askあり、資金不足、100株・取得単価確認、上級戦略の手動確認レビューを確認するための合成データです。
