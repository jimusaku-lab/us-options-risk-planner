...
export function SpreadStrategyCandidates({ candidates, ledger, simulations, onCommit }: Props) {
  ...
  const contractState = strategyContractState(selectedCandidate);
  const strategyLabel = contractState === "unknown" || !selectedCandidate.strategyType ? "2脚の組み合わせ" : getStrategyLabel(selectedCandidate.strategyType);
  ...
  return <section aria-label="現在の建玉取込" className="rounded-md border border-teal-300 bg-teal-50 p-3">
    ...
    <div className="mt-3 rounded-md border border-teal-200 bg-white p-3"><p className="text-sm font-bold">{ticker} / {strategyLabel} / {selectedCandidate.contracts}組</p>{contractState === "unknown" ? <p className="mt-1 text-xs text-amber-900">2脚として管理／契約仕様未照合</p> : null}<dl className="mt-2 grid gap-2 text-xs sm:grid-cols-2">{selectedCandidate.fills.map((fill) => { const execution = resolveStrategyFillExecution(fill, simulations); const feeSource = execution?.commissionSource === "saxo_derived_same_currency_booked_difference" ? "Saxo同一通貨の記帳差額（導出）" : execution?.commissionSource === "saxo_actual" ? "Saxo明示費用" : "費用出所未確認"; return <div key={fill.key}><dt className="font-bold">{fill.contract.optionType === "call" ? "C" : "P"}{fill.contract.side === "buy" ? "買い" : "売り"} {fill.contract.strike} / {fill.contract.expiry}</dt><dd className="mt-0.5 text-slate-700">{execution?.tradeDate ?? "約定日未取得"} / {execution?.contracts ?? "数量未取得"}枚 / 単価 {execution?.fillPriceUSD ?? "未取得"} USD / 開始費用 {execution?.commissionUSD ?? "未取得"} USD（{execution?.commissionUSD === undefined ? "未取得" : feeSource}） / {fill.existing ? "既存記録を参照" : "取得済み約定"}</dd></div>; })}</dl></div>
