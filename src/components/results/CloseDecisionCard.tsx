    ...
    <section id="close-decision" className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {isOpen ? (
        <>
          ...
          {bearPutEstimate ? <div className="mt-3 grid gap-2 rounded-md border border-indigo-200 bg-indigo-50 p-3 sm:grid-cols-3"><div><div className="text-xs text-indigo-700">建玉時手数料込み支払額</div><div className="font-bold">{bearPutEstimate.entryAllInDebitUSD !== undefined ? formatUSD(bearPutEstimate.entryAllInDebitUSD) : "未確認"}</div></div><div><div className="text-xs text-indigo-700">今閉じた場合の手数料後受取額</div><div className="font-bold">{bearPutEstimate.kind === "available" ? formatUSD(bearPutEstimate.closeNetProceedsUSD) : "未計算"}</div></div><div><div className="text-xs text-indigo-700">概算損益 / 期間損益率</div><div className="font-bold">{bearPutEstimate.kind === "available" ? `${bearPutEstimate.totalEstimatedPnlUSD >= 0 ? "+" : ""}${formatUSD(bearPutEstimate.totalEstimatedPnlUSD)} / ${bearPutEstimate.periodReturnPct >= 0 ? "+" : ""}${formatPct(bearPutEstimate.periodReturnPct)}` : bearPutEstimate.reasons.join(" / ")}</div></div></div> : null}
