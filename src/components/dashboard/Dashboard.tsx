...
export function BulkOptionPricePreview({ rows, stockRows, diagnostics, simulations, referenceConfirmed, onReferenceConfirmedChange, onApply, currentFx }: { rows: CurrentOptionPricePreviewRow[]; diagnostics: Map<string, PriceUpdateDiagnostic>; stockRows: CurrentStockPricePreviewRow[]; simulations: TradeSimulation[]; referenceConfirmed: boolean; onReferenceConfirmedChange: (checked: boolean) => void; onApply?: () => void; currentFx?: FxQuote | null }) {
  ...
  return <div className="mt-3 overflow-x-auto">
    <p className="mb-2 text-xs">API価格応答 {rows.filter(row => row.candidate !== undefined).length}脚 / 中間値評価可能 {rows.filter(row => row.referenceQuote?.kind === "available").length}脚 / 中間値使用確認待ち {rows.filter(row => row.referenceQuote?.kind === "confirmable_reference").length}脚（保存件数ではありません）</p>
...
export function Dashboard({
  ...
  onFetchBulkOptionPrices?: (simulationId?: string) => void;
  onApplyBulkOptionPrices?: (simulationId?: string) => void;
  ...
  return (
    <section id="position-dashboard" tabIndex={-1} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm focus:outline-none">
      ...
      {isBulkDialogOpen ? <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-950/40 p-4 pt-12" role="presentation">
