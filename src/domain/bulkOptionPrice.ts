...
import { recordParentTimeValueObservation } from "@/domain/timeValueParent";
...
export function createCurrentOptionPricePreviewRow(target: CurrentOptionPriceTarget, candidate?: SaxoOptionPremiumCandidate): CurrentOptionPricePreviewRow { const selected = selectSideAwarePremiumCandidate(candidate, target.side); const referenceQuote = candidate?.status === "available" ? resolveEvaluationQuote({ bid: candidate.bid, ask: candidate.ask, priceTypeBid: candidate.quoteDiagnostics?.priceTypeBid, priceTypeAsk: candidate.quoteDiagnostics?.priceTypeAsk, fetchedAt: candidate.fetchedAt, source: candidate.source, sourceTimestamp: candidate.sourceTimestamp, delayedByMinutes: candidate.quoteDiagnostics?.delayedByMinutes }) : undefined; return { target, candidate, ...selected, referenceQuote,
    ...
    ...(referenceQuote?.kind !== "unavailable" && referenceQuote?.mid !== undefined && selected.selectedPriceUSD === undefined ? { status: referenceQuote.kind === "confirmable_reference" ? "confirmable_reference" as const : "ready" as const, reason: "中間値参考評価のみ（決済候補は未採用）" } : {}) }; }
...
export function applyCurrentOptionPricePreview(simulations: TradeSimulation[], rows: CurrentOptionPricePreviewRow[], options: { includeConfirmedReferences?: boolean; capturedAt?: string; batchId?: string; currentFx?: FxQuote | null } = {}): TradeSimulation[] {
  ...
  return simulations.map((simulation) => {
    ...
    const optionLegs = simulation.optionLegs.map((leg) => {
      ...
      const resolvedFee = resolveCloseCommissionUSD(simulation, leg, Math.abs(row.target.quantity));
      const valueSnapshots = snapshot ? upsertOptionValueSnapshot(leg.valueSnapshots, snapshot) : leg.valueSnapshots;
