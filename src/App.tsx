...
export default function App() {
  ...
  const createSyntheticForwardFromSaxoPositions = (pair: SaxoSyntheticForwardPair, historyItems: SaxoHistoryDiscoveryItem[] = saxoHistoryCandidates, openEditor = true) => {
    ...
    if (existing) {
      if (shouldRecoverSaxoSyntheticForwardEntryConfirmation(existing, isFilled) && fillEvidence.callHistory && fillEvidence.putHistory) {
        const callLeg = existing.optionLegs.find((leg) => leg.saxoPositionId === (pair.callPosition.positionId ?? pair.callPosition.id));
        const putLeg = existing.optionLegs.find((leg) => leg.saxoPositionId === (pair.putPosition.positionId ?? pair.putPosition.id));
        ...
        const upgraded: TradeSimulation = {
          ...
          optionLegs: existing.optionLegs.map((leg) => leg.id === callLeg?.id && !leg.premiumUSD ? { ...leg, premiumUSD: fillEvidence.callHistory?.price ?? leg.premiumUSD } : leg.id === putLeg?.id && !leg.premiumUSD ? { ...leg, premiumUSD: fillEvidence.putHistory?.price ?? leg.premiumUSD } : leg),
          ...
          optionEntryExecutions: [...(existing.optionEntryExecutions ?? []), ...(callLeg && !existingEntryLegIds.has(callLeg.id) ? [createEntryExecution(callLeg.id, fillEvidence.callHistory, callLeg.quantity)] : []), ...(putLeg && !existingEntryLegIds.has(putLeg.id) ? [createEntryExecution(putLeg.id, fillEvidence.putHistory, putLeg.quantity)] : [])],
          fixtureMeta: { ...existing.fixtureMeta!, notes: "Saxo SyntheticUnderlying親注文と二脚の約定履歴を照合して建玉中として反映しました。3-Aで約定確認を保存してください。" },
        };
