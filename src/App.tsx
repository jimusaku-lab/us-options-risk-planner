...
export default function App() {
  ...
  const createSyntheticForwardFromSaxoPositions = (pair: SaxoSyntheticForwardPair, historyItems: SaxoHistoryDiscoveryItem[] = saxoHistoryCandidates) => {
    ...
    if (existing) {
      if (isFilled && existing.status === "planned" && fillEvidence.callHistory && fillEvidence.putHistory) {
        ...
        const upgraded: TradeSimulation = {
          ...existing, status: "open", name: existing.name.replace("API取込下書き", "Saxo約定確認"), entryDate,
    ...
    upsertSimulation(simulation); selectSimulation(simulation.id); setIsEditorOpen(true); setEditorFocusRequest({ anchorId: "option-entry-executions", requestId: Date.now() }); setQuoteStatus(isFilled ? `Saxo親注文 ${parentOrderId ?? ""} と二脚の約定履歴を根拠に、C${pair.strike}買い/P${pair.strike}売りを建玉中の3-A約定確認データへ反映しました。` : parentHistory ? `Saxo親注文 ${parentOrderId ?? ""} のネット約定価格を根拠に、C${pair.strike}買い/P${pair.strike}売りを一件のSynthetic Forward下書きへ反映しました。` : "C買い/P売りを一件のSynthetic Forward下書きへ統合しました。親注文の約定実績を3-Aで確認してください。");
  };
